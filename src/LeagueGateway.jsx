...
async function requireUserOrSignInAnonymouslyForDev() {
...
export default function LeagueGateway() {
  ...
  // On mount, sign in (anon if needed), load their leagues
  useEffect(() => {
    (async () => {
      const u = await getOrInitUser();
      setUser(u);
  ...
  async function handleCreateLeague(e) {
    ...
    await setDoc(memberRef, {
      displayName: "Guest", // we'll let them change later
      totalPoints: 0,
      joinedAt: serverTimestamp(),
    });

    // 3. push leagueId into /users/{uid}.leagues
  ...
  async function handleJoinLeague(e) {
    ...
    // 2. push leagueId into /users/{uid}.leagues
    const userRef = doc(db, "users", user.uid);
    await updateDoc(userRef, {
      leagues: arrayUnion(leagueId),
    });
